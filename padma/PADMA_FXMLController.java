...
public class PADMA_FXMLController implements Initializable{
    ...
    @FXML private Label pathCost;
    @FXML private Label expandedNode;
    @FXML private Label exploredNode;
    @FXML private Label nodeValues;
    @FXML private Label loadMap;
   ...
   // FileChooser fileChooser = new FileChooser();
    ...
    private void runAstar(ActionEvent event) {
        if(!mapLoaded){
            // temp code
            loadMap.setText("Please Load a Map");
        ...
        String pathFile=tmp[0]+" "+hu.getValue().toString()+" Weight_"+w.getText()+".txt";
        int h=0;
        //temp code
        ...
        //temp code
        pathCost.setText("Path Cost " +new BigDecimal(extendedMap[goal.y][goal.x].g).setScale(2, RoundingMode.HALF_UP).doubleValue()  );
        pathCost.setVisible(true);
        expandedNode.setText("Nodes expanded:"+astar.expandedNodeCount);
        expandedNode.setVisible(true);
        exploredNode.setText("Nodes explored:"+astar.exploredNodeCount);
        exploredNode.setVisible(true);
    ...
    void visualizeMap(){
        for(int i=0;i<120;i++){
            for(int j=0;j<160;j++){
                ...
                else if(extendedMap[i][j].status.contains("b")) r.setFill(Color.CORNFLOWERBLUE);
                ...
                r.setOnMouseClicked(new EventHandler<MouseEvent>(){
                    ...
                    public void handle(MouseEvent event) {
                            //Temp code
                            nodeValues.setText("Row: "+ GridPane.getRowIndex(r)+" Column: "+ GridPane.getColumnIndex(r)+" f-value= "+new BigDecimal(extendedMap[GridPane.getRowIndex(r)][GridPane.getColumnIndex(r)].f).setScale(2, RoundingMode.HALF_UP).doubleValue()
                                + " g-value= "+new BigDecimal(extendedMap[GridPane.getRowIndex(r)][GridPane.getColumnIndex(r)].g).setScale(2, RoundingMode.HALF_UP).doubleValue()
                                + " h-value= "+new BigDecimal(extendedMap[GridPane.getRowIndex(r)][GridPane.getColumnIndex(r)].h).setScale(2, RoundingMode.HALF_UP).doubleValue());
                            nodeValues.setVisible(true);
                    }
                    });
        ...
        highLightPath();
        Rectangle r1=new Rectangle(4,4,Color.AQUA);
